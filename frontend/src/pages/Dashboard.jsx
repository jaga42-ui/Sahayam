...
const CATEGORY_META = {
  blood: {
    ...
    bg: "bg-blazing-flame",
    soft: "bg-blazing-flame/12 text-blazing-flame border-blazing-flame/25",
  },
  food: {
    label: "Food",
    icon: FaUtensils,
    ...
    bg: "bg-pine-teal",
    soft: "bg-pine-teal/12 text-pine-teal border-pine-teal/25",
  },
  clothes: {
    label: "Clothes",
    icon: FaTshirt,
    color: "text-dark-raspberry",
    bg: "bg-dark-raspberry",
    soft: "bg-dark-raspberry/12 text-dark-raspberry border-dark-raspberry/25",
  },
  book: {
    label: "Book",
    icon: FaBook,
    ...
    soft: "bg-dusty-lavender/12 text-dusty-lavender border-dusty-lavender/25",
  },
  general: {
    label: "General",
    icon: FaHandsHelping,
    color: "text-pine-teal",
    bg: "bg-pine-teal/80",
    soft: "bg-pine-teal/8 text-pine-teal border-pine-teal/20",
  },
};

const getCategoryMeta = (c) =>
  CATEGORY_META[c?.toLowerCase()] || CATEGORY_META.general;
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const onDonorComing = (d) => {
      ...
      setTimeout(
        () => setResponders((p) => p.filter((r) => r.blastId !== d.blastId)),
        10000,
      );
    };
    const onNew = (d) => setFeed((p) => [d, ...p]);
    const onUpdated = (d) =>
      setFeed((p) => p.map((i) => (i._id === d._id ? d : i)));
  ...
  const processedFeed = useMemo(
    () =>
      feed.filter(
        (i) =>
          filterCategory === "All" ||
          i.category?.toLowerCase() === filterCategory.toLowerCase(),
      ),
    [feed, filterCategory],
  );
    ...
    () => [
      {
        ...
        icon: FaBoxOpen,
        color: "text-pine-teal",
        ring: "ring-pine-teal/20",
        bg: "bg-pine-teal/8",
      },
      {
        label: "SOS Alerts",
        value: feed.filter((i) => i.isEmergency).length,
        icon: FaHeartbeat,
        color: "text-blazing-flame",
        ring: "ring-blazing-flame/20",
        bg: "bg-blazing-flame/8",
      },
      {
        label: "Responders",
        value: feed.reduce((t, i) => t + (i.requestedBy?.length || 0), 0),
        icon: FaUsers,
        color: "text-dark-raspberry",
        ring: "ring-dark-raspberry/20",
        bg: "bg-dark-raspberry/8",
      },
      {
        label: "Completed",
        value: feed.filter((i) => i.status === "fulfilled").length,
        icon: FaCheckCircle,
        color: "text-dusty-lavender",
        ring: "ring-dusty-lavender/20",
        bg: "bg-dusty-lavender/8",
      },
    ],
  ...
  const loadMore = async () => {
    ...
    try {
      const { data } = await api.get(
        `/donations/feed?page=${page + 1}&limit=12`,
      );
        ...
        try {
          ...
          if (data?.features?.length) {
            const f =
              data.features.find(
                (f) =>
                  f.id.startsWith("place.") || f.id.startsWith("locality."),
              ) || data.features[0];
            setSosData((p) => ({
              ...p,
              addressText: f.text,
  ...
  const startVoiceRecognition = () => {
    ...
    r.onerror = ({ error }) => {
      toast.error(`Voice error: ${error}`, { id: "voice" });
  ...
  const handleSOSSubmit = async (e) => {
    ...
    try {
      ...
      const sev = ["critical", "urgent", "accident", "dying", "icu"].some((w) =>
        text.includes(w),
      )
        ? "Code Red"
        : "Code Yellow";
      ...
      if (sosData.roomNumber)
        fd.append("hospitalRoomNumber", sosData.roomNumber);
      fd.append(
        "patientDetails",
        JSON.stringify({
          name: sosData.patientName,
          age: sosData.patientAge ? +sosData.patientAge : undefined,
        }),
      );
      fd.append(
        "criticalDeadline",
        new Date(
          Date.now() + (sev === "Code Red" ? 4 : 12) * 3600000,
        ).toISOString(),
      );
  ...
  const handleApprove = async (donationId, receiverId) => {
    ...
    try {
      ...
      setFeed((p) =>
        p.map((i) =>
          i._id === donationId ? { ...i, status: "pending", receiverId } : i,
        ),
      );
  ...
  const handleShare = async (item) => {
    ...
    if (navigator.share) {
      ...
      window.open(
        `https://wa.me/?text=${encodeURIComponent(`${text} → ${window.location.origin}`)}`,
      );
    }
  };
  ...
  const sosField =
    "w-full rounded-xl border border-white/10 bg-white/6 px-4 py-3.5 text-sm font-medium text-white placeholder-white/30 outline-none focus:border-blazing-flame/60 focus:bg-blazing-flame/6 transition-all";
          ...
          <AnimatePresence>
            {responders.map((r, i) => (
              ...
              >
                ...
                <span className="text-xs font-bold text-pine-teal">
                  {r.donorName} is on their way
                </span>
              </motion.div>
            ))}
          </AnimatePresence>
        ...
        <div className="relative z-10 mx-auto max-w-6xl px-4 pb-28 md:pb-16">
            ...
            <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
              ...
              <div>
                ...
                >
                  ...
                  <span
                    className={
                      isDonor ? "text-blazing-flame" : "text-dark-raspberry"
                    }
                  >
              ...
              >
                ...
                >
                  {isDarkMode ? (
                    ...
                    <FaMoon className="text-dusty-lavender" />
                  )}
                </motion.button>
          ...
          >
            {stats.map((s) => {
              ...
              return (
                ...
                >
                  <div
                    className={`absolute -top-4 -right-4 h-20 w-20 rounded-full ${s.bg} blur-xl`}
                  />
                  <p className="relative text-[10px] font-black uppercase tracking-widest text-dusty-lavender mb-2">
                    {s.label}
                  </p>
                  <div className="relative flex items-end justify-between">
                    ...
                    <div
                      className={`flex h-9 w-9 items-center justify-center rounded-xl ${s.bg} ring-1 ${s.ring}`}
                    >
          ...
          <motion.div
            ...
            onClick={() => {
              ...
              window.open(
                `https://wa.me/?text=I%20just%20joined%20Sahayam.%20Come%20join%20me%3A%20${link}`,
                "_blank",
              );
            }}
            ...
            <div className="relative flex flex-col md:flex-row items-center justify-between gap-5">
              <div>
                ...
                <h3 className="text-xl md:text-2xl font-black text-white tracking-tight leading-snug">
                  Every person you invite
                  <br className="hidden md:block" /> could save a life.
                </h3>
              </div>
          ...
          {loading ? (
            ...
            >
              ...
              <h3 className="text-lg font-black text-pine-teal tracking-tight">
                Nothing here yet
              </h3>
              <p className="mt-2 max-w-xs text-sm font-medium text-pine-teal/50 leading-relaxed">
                Be the first to post in this category, or try a different
                filter.
              </p>
              <button
                onClick={() => navigate("/donations")}
                      ...
                      <div className="relative h-48 w-full shrink-0 overflow-hidden bg-pearl-beige/60">
                        {item.image ? (
                          ...
                          >
                            ...
                            <span className="relative text-5xl font-black drop-shadow-lg">
                              {item.bloodGroup || "Blood"}
                            </span>
                          </div>
                        ) : (
                          <div
                            className={`flex h-full w-full flex-col items-center justify-center gap-2 border-b ${meta.soft}`}
                          >
                      ...
                      <div className="flex flex-1 flex-col p-5">
                        ...
                        <div className="flex items-center gap-2.5 mb-3">
                          <img
                            src={
                              ...
                              `https://ui-avatars.com/api/?name=${avatarName}&background=ece8df&color=1d4a42&bold=true`
                            }
                            alt="Donor"
                          ...
                          <div className="min-w-0">
                            <p className="flex items-center gap-1 text-sm font-bold text-pine-teal truncate leading-tight">
                              ...
                              {item.donorId?.points >= 50 && (
                                <FaMedal className="shrink-0 text-[10px] text-blazing-flame" />
                              )}
                            </p>
                            <p className="text-[10px] font-medium text-dusty-lavender">
                              {dateStr}
                            </p>
                          </div>
                          {/* Status chip */}
                          <span
                            className={`ml-auto shrink-0 rounded-lg px-2.5 py-1 text-[9px] font-black uppercase tracking-widest border ${
                              item.status === "fulfilled"
                                ? "bg-pine-teal/8 text-pine-teal border-pine-teal/20"
                                : item.status === "pending"
                                  ? "bg-dark-raspberry/8 text-dark-raspberry border-dark-raspberry/20"
                        ...
                        {item.patientDetails?.name && (
                          <div className="mb-3 rounded-xl border-l-2 border-blazing-flame bg-blazing-flame/5 px-3 py-2">
                            <p className="text-[9px] font-black uppercase tracking-widest text-blazing-flame mb-0.5">
                              Patient
                            </p>
                            <p className="text-sm font-bold text-pine-teal">
                              {item.patientDetails.name}
                              {item.patientDetails.age && (
                                <span className="text-pine-teal/50 font-medium ml-1.5">
                                  · {item.patientDetails.age} yrs
                                </span>
                              )}
                            </p>
                            {item.hospitalRoomNumber && (
                              <p className="text-[11px] font-medium text-pine-teal/60 mt-0.5">
                                Ward:{" "}
                                <span className="font-bold text-dark-raspberry">
                                  {item.hospitalRoomNumber}
                                </span>
                              </p>
                            )}
                          ...
                          {isMine ? (
                            item.status === "fulfilled" ? (
                              <div className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-pine-teal/12 bg-surface-2 py-3 text-[11px] font-bold text-pine-teal/50">
                                <FaCheckCircle className="text-pine-teal/40" />{" "}
                                Completed
                              </div>
                            ) : (
                              <>
            ...
            <div className="fixed inset-0 z-[3000] flex items-end justify-center p-4 sm:items-center">
              ...
              >
                ...
                <div className="relative mb-6 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-bold uppercase tracking-widest text-blazing-flame/70">
                ...
                <form onSubmit={handleSOSSubmit} className="relative space-y-3">
                  ...
                  <input
                    placeholder="Ward / Room No. (optional)"
            ...
            <div className="fixed inset-0 z-[3000] flex items-center justify-center p-4">
              ...
              >
                <div className="mb-5 flex items-center justify-between border-b border-pine-teal/8 pb-4">
                  <div>
                    <h2 className="text-lg font-black text-pine-teal tracking-tight">
                      Incoming Requests
                    </h2>
                    <p className="text-[11px] font-medium text-dusty-lavender mt-0.5">
                      Select who receives this item
                    </p>
                  </div>
                  <button
                    onClick={() =>
                ...
                <div className="max-h-[50vh] space-y-2.5 overflow-y-auto no-scrollbar">
                  {requestsModal.donation.requestedBy.map((req) => (
                    <div
                      key={req._id}
                      className="flex items-center justify-between rounded-2xl border border-pine-teal/10 bg-surface-2 p-3.5"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-bold text-pine-teal truncate max-w-[120px]">
                          {req.name}
                        </span>
                      </div>
                      <motion.button
                        ...
                        whileTap={{ scale: 0.94 }}
                        onClick={() =>
                          handleApprove(requestsModal.donation._id, req._id)
                        }
